...
log_file="long_term_forecast_LSTM_weather_results.log"
echo "Training Results - LSTM Baseline (Weather)" > $log_file
...
echo "" | tee -a $log_file
echo "[$(date '+%Y-%m-%d %H:%M:%S')] All training completed for LSTM on Weather!" | tee -a $log_file
