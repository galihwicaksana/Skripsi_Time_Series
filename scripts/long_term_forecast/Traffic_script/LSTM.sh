...
log_file="long_term_forecast_LSTM_traffic_results.log"
echo "Training Results - LSTM Baseline (Traffic)" > $log_file
...
echo "" | tee -a $log_file
echo "[$(date '+%Y-%m-%d %H:%M:%S')] All training completed for LSTM on Traffic!" | tee -a $log_file
