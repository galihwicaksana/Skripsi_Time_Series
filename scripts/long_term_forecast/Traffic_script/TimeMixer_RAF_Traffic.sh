...
# Traffic dataset: 862 features - use larger model
e_layers=2
down_sampling_layers=3
down_sampling_window=2
learning_rate=0.01
d_model=32  # Larger for more features
...
use_raf=1
raf_d_embed=32  # Match d_model
raf_fusion_method='gating'
raf_similarity_metric='cosine'
kb_path='./checkpoints/knowledge_bases/traffic_kb.pkl'

# Log file
log_file="long_term_forecast_RAF_Traffic_results.log"
echo "RAF-TimeMixer Training Results - Traffic Dataset" > $log_file
...
for seq_len in 48 96 168 336; do
  for pred_len in 96 192 336 720; do
    ...
    model_id="Traffic_${seq_len}_${pred_len}_RAF"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] All RAF-TimeMixer training completed for Traffic!" | tee -a $log_file
